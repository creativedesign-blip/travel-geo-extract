...
description: 從旅遊 DM、行程表、行銷文案中抽取多層標籤：地理層「國家 / 地區 / 城市 / 景點」與詞組層「交通 / 飯店 / 餐食 / 價格 / 日期」。支援 .txt/.md/.docx/.pdf 與圖片 OCR 輸入。內建詞組預檢，先保留非地理旅遊標籤並避免它們被誤升級成目的地，再針對「高山火車」、「桃園機場集合」、「東方威尼斯」「越南威尼斯」「澳門威尼斯人」等歧義/角色/比喻/弱信號自動消解。觸發詞：旅遊地名抽取、旅遊DM抽取、行程抽取、旅遊文案打標、country tagging、travel NER、大都會旅遊、ddv 標籤、把這份 DM 轉成標籤、幫我抽國家地區、強弱信號判定、識別國家、抽交通飯店價格日期。
metadata:
  version: 0.2.0
...
- `tags`：地理標籤 `{國家, 地區, 城市, 景點}`
- `term_tags`：非地理旅遊詞組標籤 `{交通, 飯店, 餐食, 價格, 日期}`

同時維護背後的國家-地區知識表。

本 skill 已整合 `industry-term-composer` 對旅遊地名抽取有用的部分：在真正打地理標籤前，先做保守詞組預檢；交通產品、飯店名、餐食、日期、價格會保留在 `term_tags`，但不會只因為含有地名樣字串就被升級成目的地。
...
在候選進入正式地理標籤前，先抽出 `term_tags`，並判斷地名候選是否只是某個非目的地詞組的一部分：

- 交通產品：`高山火車`、`高山鐵路`、`高山纜車`、`JR新幹線`、`遊船`、`巴士`
- 住宿設施：`高山飯店`、`某某酒店`、`某某溫泉旅館`、`resort/hotel`
- 餐食/設施：`餐廳`、`料理`、`美食`、`早餐/午餐/晚餐`
- 價格/日期：`NT$12900`、`每人 12900 起`、`5/25`、`5月出發`、`暑假`

核心規則：

- 不因為候選含有像地名的字，就直接輸出成目的地。
- 若候選緊接交通/飯店/餐食後綴，且同一句沒有明確地理線索，保留為 `term_tags`，並在地理層標成 `exclude`。
- 若同句有強地理線索（例如 `日本高山`、`飛驒高山`、`前往高山`、`入住高山`、`D2 高山`），保留候選給後續消歧與角色判定。
- 日期與價格只能輔助判斷旅遊產品，不足以單獨證明目的地。

範例：

| 原文 | 詞組層結果 | 地理層結果 | 理由 |
|---|---|---|---|
| 搭乘高山火車欣賞山脈 | `高山火車` → transport | 濾除高山 | `火車` 是交通產品語境，無日本/飛驒/行程目的地線索 |
| 日本飛驒高山合掌村 5 日 | `5日` 可作日期/行程資訊 | 保留高山 | `日本`、`飛驒`、`合掌村` 是強地理線索 |
| 高山飯店住宿 | `高山飯店` → hotel | 濾除或低信心 | 可能只是飯店名，不能只靠飯店名標城市 |
